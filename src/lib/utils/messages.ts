...
export class MessageInterface {
	...
	unsubscribe = async (event: string, listener: (...args: any[]) => unknown) => {
		if (this.eventBus.listenerCount(event) === 1) {
			await this.sendMessage({ command: 'UNSUBSCRIBE', event });
		}
	...
	sendMessage = ({ command, event, data, args }: Record<string, any>) => {
		const nonce = uuidv4();
		window.parent.postMessage({ command, event, data, args, nonce }, document.referrer || '*');
	...
	private isValidMessageData(messageData: unknown): messageData is MessageData {
		return typeof messageData === 'object' && !Array.isArray(messageData) && messageData !== null;
	}
}
